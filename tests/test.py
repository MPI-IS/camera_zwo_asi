    ...
    for controllable, instance in original_controls.items():
        if instance.is_writable:
            if instance.is_auto:
                ...
                assert (
                    original_controls[controllable].value
                    == new_controls[controllable].value
                )
...
def test_update_camera_config():
    ...
    for instance in controls.values():
        if instance.supports_auto:
            ...
            instance.value = int((instance.max_value + instance.min_value) / 2.0)
        ...
        if instance.is_writable:
            if instance.supports_auto:
                ...
                assert instance.value == int(
                    (instance.max_value + instance.min_value) / 2.0
                )
